...
#' @details the input dataset must contain thermalTime column
...
#'  resu<-fitLocfit(datain=plant1,trait="biovolume",myylim=700,tt=myThermalTimes,reference="Ref")
#'  str(resu)
#' }
#' @export
fitLocfit<-function(datain,trait,myylim,tt,reference){
  ...
  for (i in levels(factor(datain[,reference]))){
    ...
    if (length(x) <=4){
      plot(x,y,main=i,xlab="Thermal time",ylab=trait,ylim=c(0,myylim),xlim=c(0,60))
      next
    }
    else if (length(x) <= 7) {
      mylocfit<-30
...
#' \donttest{
#' # Take a while...
#'  resu<-fitReg(datain=plant2,trait="F_visible",myylim=22,reference="Ref")
#'  str(resu)
#' }
#' @export
fitReg<-function(datain,trait,myylim,reference){
  datain<-as.data.frame(datain)
  myfm<-list()
  tpphy<-numeric()
  tabpred<-NULL    # dataframe
  for (i in levels(factor(datain[,reference]))){
    ...
    # Graph fit model
    plot(x,y,main=i,xlab="Thermal time",ylab=trait,ylim=c(0,myylim),
         xlim=c(0,60))
